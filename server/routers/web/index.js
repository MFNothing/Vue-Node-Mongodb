module.exports = app => {
    ...
    router.get('/news/list', async (req, res) => {
        // populate 好用，但是存在缺陷，当我们想到均匀查到分类中的内容时，可能第一个分类就查到足量的内容了
        // const parent = await Category.findOne({
        //     name: '新闻分类'
        // }).populate({
        //     path: 'children',
        ...
        const cats = await Category.aggregate([
            ...
            {
                $lookup: {
                    from: 'articles', // 集合名字，默认是表名的小写复数形式
                    localField: '_id', // 本地键
                    foreignField: 'categories', // 外键
                    as: 'newsList' // 查出后放入的字段，必传
                }
            },
            {
                $addFields: { // 新增字段，但是我们已经有newList字段了，就变成修改，
                    //$slice筛选操作符，第一个参数表示筛选的字段，第二个参数是数量
                    newsList: { $slice: ['$newsList', 5] }
                }
            }
        ])
        // 热门分类不是新增分类，由现在分类拼接出来的
        // unshit 在数组头操作
    ...
    router.get('/heroes/init', async (req, res) => {
        ...
        console.log(rowData[0].heroes.map(v => {
            return v.name
        }))
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            ...
            {
                $lookup: {
                    from: 'heroes', // 集合名字，默认是表名的小写复数形式
                    localField: '_id', // 本地键
                    foreignField: 'categories', // 外键
                    as: 'heroList' // 查出后放入的字段，必传
                }
            }
        ])
        // 热门分类不是新增分类，由现在分类拼接出来的
        ...
        cats.map(cat => {
            cat.heroList.map(heroes => {
                heroes.categoryName = (cat.name === '热门')
                    ? heroes.categories[0].name : heroes.name
                return heroes
            })
            return cat
        })
...
// chrome 里面操作
// $(selector, [startNode]) 语法
// $$('li', $$('.hero-list')[i]) 表示从所有li中筛选出 .hero-list下的dom
// $$('.hero-list') 会有多个，对应英雄分类的个数
/*
$$('.hero-nav > li').map((v, i) => {
    return {
        name: v.innerText,
